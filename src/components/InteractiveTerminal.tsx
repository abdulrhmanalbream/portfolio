...
export function InteractiveTerminal() {
  ...
  const handleCommand = (cmdStr: string) => {
    ...
    switch (trimmed) {
      ...
      case "ping":
        const randTime = (Math.random() * 20 + 15).toFixed(1);
        out = (
          <div className="space-y-1 mt-1 text-xs font-mono text-zinc-400">
            <p>{TERMINAL_CONTENT.pingMessage.header}</p>
            <p>{TERMINAL_CONTENT.pingMessage.packetFormat.replace("{{seq}}", "1").replace("{{time}}", randTime)}</p>
            <p>{TERMINAL_CONTENT.pingMessage.packetFormat.replace("{{seq}}", "2").replace("{{time}}", (parseFloat(randTime) - 1.2).toFixed(1))}</p>
            <p className="text-matrix-green">{TERMINAL_CONTENT.pingMessage.stats}</p>
            <p>2 packets transmitted, 2 received, 0% packet loss, time={randTime}ms</p>
            <p>rtt min/avg/max/mdev = {(parseFloat(randTime) - 1.2).toFixed(1)}/{randTime}/{randTime} ms</p>
          </div>
        );
        break;
  ...
  return (
    ...
    <div className={`bg-black/90 border border-matrix-green/20 rounded-lg flex flex-col font-mono text-xs select-text shadow-xl relative ${
      isMaximized 
